...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // ==========================
      // ✅ ÚNICO CAMBIO: ENVIAR EMAIL (como en la versión anterior)
      // ==========================
      ...
      // si no hay correo, no intentamos enviar (evita fallo del template)
      if (correo) {
        await emailjs.send("service_14jwpyq", "template_itx9f7f", templateParams);
      } else {
        console.warn("No se envió email: el cliente no ingresó correo.");
      }
      // ==========================
