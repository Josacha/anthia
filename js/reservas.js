...
import { collection, getDocs, query, where, addDoc, doc, setDoc, Timestamp } 
...
async function cargarServicios() {
  ...
  snapshot.forEach(docSnap => {
    ...
    option.textContent = `${data.nombre} - ₡${data.precio} (${data.duracion || 1} min)`;
...
function renderCarrito() {
  ...
  carrito.forEach((s, i) => {
    ...
    div.innerHTML = `<span>${s.nombre} (${s.duracion} min)</span> 
...
async function cargarHorasDisponibles() {
  ...
  HORAS.forEach(hora => {
    ...
    for (let i = 0; i < duracionTotal; i += 30) {
      const minutoCheck = horaInicio + i;

      // Contar cuántas citas ocupan este bloque
      const citasEnBloque = citas.filter(cita => {
        const [ch, cm] = cita.hora.split(":").map(Number);
        const inicioCita = ch * 60 + cm;
        const durCita = cita.servicios.reduce((acc, s) => acc + s.duracion, 0);
        return minutoCheck >= inicioCita && minutoCheck < inicioCita + durCita;
      }).length;

      // Si hay 2 o más citas, bloquear aunque haya simultaneidad
      if (citasEnBloque >= 2) {
        disponible = false;
        break;
      }
...
formReserva.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    for (const s of carrito) {
      ...
      horaIndex += Math.ceil(s.duracion / 60); // Avanzar por duración en bloques de 1h
    }
